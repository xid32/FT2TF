...
from models.loss import compute_reconstruction_loss, compute_perceptual_loss
...
def save_checkpoint(model, optimizer, step, checkpoint_dir, epoch, hparams, global_step):
...
def audio_metrics(g_mel, gt_mel, dataset, mel_len, stft, folder):
    wav_pred = dataset.inverse_mel(g_mel.detach(), mel_len, stft)
    wav_tr = dataset.inverse_mel(gt_mel.detach(), mel_len, stft)
    stoi_list, estoi_list, pesq_list = [], [], []
    for _ in range(g_mel.size(0)):
        min_len = min(len(wav_pred[_]), len(wav_tr[_]))
        sf.write(f'{folder}/a_{_}_g.wav', wav_pred[_][:min_len], 16000, subtype='PCM_16')
        sf.write(f'{folder}/a_{_}_gt.wav', wav_tr[_][:min_len], 16000, subtype='PCM_16')
        ...
        try:
            pesq_list.append(pesq(16000, wav_tr[_][:min_len], wav_pred[_][:min_len], 'nb'))
        except:
            continue
    mean_stoi = np.mean(np.array(stoi_list))
    mean_estoi = np.mean(np.array(estoi_list))
    mean_pesq = np.mean(np.array(pesq_list))
    return mean_stoi, mean_estoi, mean_pesq


def eval_sample(g, gt, au, aut, global_step, checkpoint_dir, dataset, mel_len, stft, save_v=True, save_a=False):
    folder = join(checkpoint_dir, "samples_step{:09d}".format(global_step))
    if not os.path.exists(folder): os.mkdir(folder)
    ...
    mean_stoi, mean_estoi, mean_pesq = audio_metrics(au, aut, dataset, mel_len, stft, folder)
    with open(f'{folder}/sample_results.txt', 'w') as f:
        f.write(f'SSIM : {average_ssim}\n')
        f.write(f'PSNR : {average_psnr}\n')
        f.write(f'STOI : {mean_stoi}\n')
        f.write(f'ESTOI : {mean_estoi}\n')
        f.write(f'PESQ : {mean_pesq}\n')
    ...
    if save_a:
        ...
        for batch_idx, c in enumerate(au):
            for t in range(len(c)):
                ...
                plt.colorbar(format='%+2.0f dB')
                plt.ylabel('mel-spectrogram channels')
                plt.savefig('{}/a_{}_{}_g.png'.format(folder, batch_idx, t))

        for batch_idx, c in enumerate(aut):
            for t in range(len(c)):
                mel_spectrogram = np.log(c[t] + 1e-10)
                plt.figure()
                plt.imshow(np.squeeze(mel_spectrogram), cmap='inferno', origin='lower', aspect='auto')
                plt.colorbar(format='%+2.0f dB')
                plt.ylabel('mel-spectrogram channels')
                plt.savefig('{}/a_{}_{}_gt.png'.format(folder, batch_idx, t))
    return average_ssim, average_psnr, mean_stoi, mean_estoi, mean_pesq
