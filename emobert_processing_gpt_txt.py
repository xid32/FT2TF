...
def get_image_list(data_root, video_data_root, split):
    ...
    with open('/jumbo/jinlab/XD/face_expression/project/filelists/{}.txt'.format(split)) as f:
        for line in f:
            ...
            if not os.path.exists(os.path.join(video_data_root, line+"_gpt.txt")):
                continue
            filelist.append(os.path.join(data_root, line))
            linelist.append(line)
            with open(os.path.join(video_data_root, line+"_gpt.txt"), 'r') as fline:
...
for split in ['train', 'test', 'val']:
    ...
    for idx, vidname in tqdm(enumerate(all_videos), total=len(all_videos)):
        ...
        np.save(os.path.join(args.video_data_root, linelist[idx] + "_roberta_gpt_txt.npy"), encoded_texts)
