...
from models.model_final import LLCFaceSy, LLCFaceSy_ablation_global, LLCFaceSy_ablation_local, LLCFaceSy_ablation_att, LLCFaceSy_vit_small, LLCFaceSy_vit_large
from utils.train_save import save_checkpoint, eval_sample
import wandb
import warnings
from models.discriminator import LLCFaceSy_discriminator
from utils.generation_eval import eval_test
...
def train(device, model, disc_model, train_data_loader, test_data_loader, test_data_line_list, optimizer, disc_optimizer, args,
          checkpoint_dir=None, checkpoint_interval=None, nepochs=None):
    global global_step, global_epoch

    if args.test_only:
        print("TEST ONLY MODE!!!")
        test_res = eval_test(test_data_loader, model, checkpoint_dir, global_step, test_data_line_list, save_test=True)
        return
...
def main():
    ...
    test_dataset = Dataset(args, 'test', mode="test")
    ...
    test_data_loader = data_utils.DataLoader(
        ...
        num_workers=args.num_workers, prefetch_factor=args.prefetch_factor,
        persistent_workers=args.persistent_workers, pin_memory=args.pin_memory)
    test_data_line_list = test_dataset.get_line_list()
    device = torch.device("cuda" if args.use_cuda else "cpu")
    # Model
    if args.vit_small:
        model = LLCFaceSy_vit_small()
    elif args.vit_large:
        model = LLCFaceSy_vit_large()
    elif args.global_exp and not args.local_exp:
        model = LLCFaceSy_ablation_global()
    elif args.local_exp and not args.global_exp:
        model = LLCFaceSy_ablation_local()
    elif args.local_exp and args.global_exp:
        model = LLCFaceSy_ablation_att()
    else:
        model = LLCFaceSy()
    ...
    if args.checkpoint_path is not None:
        model = load_checkpoint(args.checkpoint_path, model, optimizer, reset_optimizer=True, overwrite_global_states=True)
    disc_model = load_checkpoint("./ckpts/discriminator_pretrained.pth", disc_model, disc_optimizer,
                    reset_optimizer=True, overwrite_global_states=False)
    if args.disc_checkpoint_path is not None:
        disc_model = load_checkpoint(args.disc_checkpoint_path, disc_model, disc_optimizer,
    ...
    print("Training...")
    train(device, model, disc_model, train_data_loader, test_data_loader, test_data_line_list, optimizer, disc_optimizer, args,
