...
# splits = ['pretrain'] 
splits = ['lrs3_testfilelists']
with torch.no_grad():
    for split in splits:
        video_data_root = args2.video_data_root
        data_root = args2.data_root
        if split == "pretrain":
            video_data_root = video_data_root.replace("main", "pretrain")
            data_root = data_root.replace("main", "pretrain")
        textlist = []
        with open('lrs3_testfilelists/{}.txt'.format(split)) as f_:
            f = list(f_)
